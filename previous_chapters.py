...
# Chapter 5
#####################################
def generate( #MODIFIED FOR IMPROVEMENTS ON DECODING
    model,
    idx,
    max_new_tokens,
    context_size,
    ...
    for _ in range(max_new_tokens):
        ...
        # Use only tokens that fit inside the model's context window
        idx_cond = idx[:, -context_size:]

        with torch.no_grad():
            logits = model(idx_cond)

        # Keep the logits for the next token only
        logits = logits[:, -1, :]
        ...
        if repetition_penalty != 1.0:
            for batch_index in range(idx.shape[0]):
                ...
                previously_generated_tokens = torch.unique(
                    idx[batch_index]
                )
        ...
        if top_k is not None:
            ...
            minimum_top_logit = top_logits[:, -1].unsqueeze(-1)

            logits = torch.where(
                logits < minimum_top_logit,
        ...
        if (
            eos_id is not None
            and idx_next.shape[0] == 1
            and idx_next.item() == eos_id
        ):
            break
